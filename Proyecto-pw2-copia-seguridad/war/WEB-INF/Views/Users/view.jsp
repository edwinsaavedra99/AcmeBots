	                ...
	                <div class="row">
	                    <div class="col-sm-6">
						 ...
						 <div class="col-sm-6">
	                        ...
	                        <a href="/users/edit?id=<%= user.getId() %>" class="btn btn-warning" data-toggle="modal"><i class="material-icons">&#xe254;</i> <span>Edit User</span></a>
	              ...
	              <table class="table table-striped table-hover table-bordered">
	                <form>
	                    <div class="modal-header">                      
	                        <h4 class="modal-title">View User <b><%=user.getId()%></b></h4>
	                    </div>
	                    <table  align="center">
	                        <tr>
	                            <th><div  class="modal-body" >
	                                <img src="https://scontent.faqp2-1.fna.fbcdn.net/v/t1.0-9/10944846_397274003778665_6749088264138891970_n.jpg?_nc_cat=0&oh=af74479921e77def5abd9b921e07998a&oe=5B7BC927"  style="width:550px;height:450px;"> 
	                    	        </div>
	                    	    </th>
	                            <th>
	                              <div class="modal-body" >                   
	                                <div class="form-group">
	                                    ...
	                                    <input type="text"  value="<%=user.getIdRole()%>" class="form-control" size="30" maxlength="30" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>Name</label>
	                                    <input type="text" value="<%=user.getName()%>" class="form-control" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>Last Name</label>
	                                    <input type="text" value="<%=user.getLastName()%>" class="form-control" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>E-mail</label>
	                                    <input type="email" value="<%=user.getEmail()%>" class="form-control" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>Phone</label>
	                                    <input type="number" value="<%=user.getPhone()%>" class="form-control" disabled>
	                                </div>
	                                 <div class="form-group">
	                                    <label>Address</label>
	                                    <input type="text" value="<%=user.getAddress()%>" class="form-control" disabled>
	                                </div>
	                                 	<div class="form-group">
                                    	<label>Status</label><br>
                                    	<input name="status" type="radio" id="actived" value="true" <% if (user.isStatus()==true)%><%{%> checked<%}%> disabled> Actived<br>
                                    	<input name="status" type="radio" id="disabled" value="false" <% if (user.isStatus()==false)%><%{%>checked <%}%> disabled> Disabled
                                		</div>                  
	                           		 </div>
	                            </th>
	                          </tr>
