            	...
            	<div class="table-title">
                	<div class="row">
                    	...
                    	<div class="col-sm-6">
    	                    ...
    	                    <a href="/roles/view?id=<%= role.getId() %>" class="btn btn-warning" data-toggle="modal"><i class="material-icons">&#xe8f4;</i><span>View Role</span></a>
                        <a href="/roles/delete?id=<%= role.getId() %>" class="btn btn-primary" onclick="return confirmar()" data-toggle="modal"><i class="material-icons">&#xe14c;</i> <span>Delete Role</span></a>
        	            </div>
            	    </div>
            	</div>
            	<br>
            	<table class="table table-striped table-hover table-bordered">
                	<form method="post" action="/roles/edit?id=<%=role.getId()%>">
	                    <div class="modal-header">                      
    	                    <h4 class="modal-title">Edit Role <%= role.getId() %></h4>
                        </div>
                        <table  align="center">
                        	<tr>
                            	<th>
                                ...
                                <th>
                            		<div class="modal-body" >                   
	                                	<div class="form-group">
                                    		...
                                    		<input name="nameRole" type="text"  value="<%=role.getNameRole()%>"  class="form-control" size="30" maxlength="30" required>
                                		</div>
                                		<div class="form-group">
                                    		<label>Description</label>
                                    		<textarea name="description" rows="5" cols="15" class="form-control" required><%=role.getDescription()%></textarea>
                                    	
                                		</div>
                                		<div class="form-group">
                                    		<label>Status</label><br>
                                    		<input name="status" type="radio" id="actived" value="true"  <% if (role.isStatus()==true)%><%{%> checked<%}%>> Actived<br>
                                    		<input name="status" type="radio" id="disabled" value="false" <% if (role.isStatus()==false)%><%{%> checked<%}%>> Disabled
                                		</div>
            	        ...
            	        <div class="modal-footer">
            	        	<a href="/roles/edit?id=<%=role.getId()%>">
							<input type="button" class="btn btn-default" data-dismiss="modal" value="Defauld">				
						</a>
                	        <input type="submit" id="btsubmit" class="btn btn-success" value="Edit Role">
                    	</div>
                </form>
            </table>
         </div>
