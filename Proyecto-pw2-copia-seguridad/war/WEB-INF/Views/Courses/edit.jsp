            ...
            <div class="table-title">
                <div class="row">
					...
					<div class="col-sm-6">
                        ...
                        <a href="/courses" class="btn btn-info" data-toggle="modal"><i class="material-icons">&#xe152;</i> <span>List Courses</span></a>
                        <a href="/courses/view?id=<%= course.getId() %>" class="btn btn-warning" data-toggle="modal"><i class="material-icons">&#xe8f4;</i><span>View</span></a>
                        <a href="/courses/delete?id=<%= course.getId() %>" onclick="return confirmar()" class="btn btn-primary" data-toggle="modal"><i class="material-icons">&#xe14c;</i> <span>Delete</span></a>
                    </div>
                </div>
            </div>
            <br>
				<table class="table table-striped table-hover table-bordered">
					<form method="post" action="/courses/edit?id=<%=course.getId()%>"> 
							<div class="modal-header">						
								<h4 class="modal-title">Edit Course <%= course.getId() %></h4>
							</div>
							<table  align="center">
								<tr>
							...
							<div class="modal-body" >					
								...
								<div class="form-group">
									<label>Name</label>
									<input type="text" class="form-control"  name="nameCourse" value="<%=course.getCourseName()%>" required>
								</div>
												
								<div class="form-group">
									<label>Rugro</label>
									<input type="text" class="form-control" name="rugro" value="<%=course.getRugro()%>" required>
								</div>
								<div class="form-group">
									<label>Duracion</label>
									Semanas <input type="number" class="form-control" name="duracion" value="<%=course.getDuracion()%>" required>
								</div>
								<div class="form-group">
                                    <label>Description</label>
                                    <textarea name="description" rows="5" cols="15" class="form-control" required><%=course.getDescription()%></textarea>
                                </div>
								<div class="form-group">
									<label>Banner</label>
									<input type="text" class="form-control" name="banner" value="<%=course.getBanner()%>" required>
								</div>
								<div class="form-group">
                                    <label>Status</label><br>
                                    <input name="status" type="radio" id="actived" value="true"  <% if (course.isStatus()==true)%><%{%> checked<%}%>> Actived<br>
                                    <input name="status" type="radio" id="disabled" value="false" <% if (course.isStatus()==false)%><%{%> checked<%}%>> Disabled
                                </div>					
					...
					<div class="modal-footer">
						<a href="/courses/edit?id=<%=course.getId()%>">
