...
<head>
	...
	<script>
		function confirmar(){
			if(confirm('¿Estas seguro que quieres borrar a este Curso ?'))	
				return true;
			else
				return false;
		}
        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        ...
                        <a href="/courses/add" class="btn btn-success"   data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Add New Course</span></a>
			...
			<% if (courses.size() > 0) { %>
				<% for (int i = 0;i<courses.size();i++) { %>
					<% Courses c = (Courses)courses.get(i); %>
					<tr>
                    	<td><%= i+1 %></td>
                    	<td><%= c.getId() %></td>
						...
						<td><%= c.getRugro() %></td>
                        <td><%= c.getDuracion() %></td>
                        <td><%= c.getDateInicio() %></td>
                        <td><%= c.isStatus() %></td>
                    	    <td>
	                    	   <a href="courses/view?id=<%=c.getId()%>" class="view" title="View" data-toggle="tooltip"><i class="material-icons">&#xE417;</i></a>
                               <a href="courses/edit?id=<%=c.getId()%>" class="edit" title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a>
                               <a href="courses/delete?id=<%=c.getId()%>"  onclick="return confirmar()" class="delete" title="Delete" data-toggle="tooltip"><i class="material-icons">&#xe14c;</i></a>
                                <a href="courses/actividad?id=<%=c.getId()%>" class="delete" title="Actividad" data-toggle="tooltip"><i class="material-icons">&#xE417;</i></a>
                            </td>
            		</tr>
            	<%} %>   	 
			<%} %>   
