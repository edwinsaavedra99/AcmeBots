	              ...
	              <table class="table table-striped table-hover table-bordered">
	                <form>
	                    <div class="modal-header">                      
	                        <h4 class="modal-title">View User <b><%=course.getId()%></b></h4>
	                    </div>
	                    <table  align="center">
	                        <tr>
	                            <th><div  class="modal-body" >
	                                <img src="<%=course.getBanner()%>"  style="width:550px;height:450px;"> 
	                    	        </div>
	                    	    </th>
	                            <th>
	                              <div class="modal-body" >                   
	                                <div class="form-group">
	                                    <label>Name</label>
	                                    <input type="text"  value="<%=course.getCourseName()%>" class="form-control" size="30" maxlength="30" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>Rugro</label>
	                                    <input type="text" value="<%=course.getRugro()%>" class="form-control" disabled>
	                                </div>
	                                <div class="form-group">
	                                    <label>Duration</label>
	                                    <input type="text" value="<%=course.getDuracion()%>" class="form-control" disabled>
	                                </div>
	                                <div class="form-group">
                                    	<label>Description</label>
                                    	<textarea name="description" rows="5" cols="15"  class="form-control" disabled><%=course.getDescription()%></textarea>
                                	</div>
	                               	<div class="form-group">
                                    	<label>Status</label><br>
                                    	<input name="status" type="radio" id="actived" value="true" <% if (course.isStatus()==true)%><%{%> checked<%}%> disabled> Actived<br>
                                    	<input name="status" type="radio" id="disabled" value="false" <% if (course.isStatus()==false)%><%{%>checked <%}%> disabled> Disabled
                                		</div>                  
	                           		 </div>
	                           		 <div class="form-group">
